...
- **대가는 톤이 아니다 — `money`·`bulk` 는 승격 인자에서 뺀다**(v1.20). 그 자리를 무엇으로
  ...
  - **확인 게이트 의무 — 되돌릴 수 없는 일괄에만**(v1.21 에서 범위를 좁힘). `bulk` 이면서
    ...
    | 조건 | 뜻 | 예 |
    |---|---|---|
    | **가. 덮어쓰기·소실형** | 여러 건의 기존 입력·데이터를 한꺼번에 덮어쓰거나 지운다 | popo 전체 프롬프트 다듬기(손입력 문구를 AI 문구로 일괄 대체) |
    ...
    - **게이트 문안에 금액을 쓰지 않는다** — 말하는 것은 **범위(건수)와 되돌릴 수 없음**뿐이다
      (위 비용 비노출 결정과 같은 이유).
    - **같은 버튼을 다시 눌러 회복되는 일괄은 대상이 아니다**. 실패해도 재시도로 끝나는 것에
      게이트를 달면 확인창이 흔해져 진짜 확인이 안 읽힌다(예: popo 이미지 생성).
