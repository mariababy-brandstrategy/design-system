...
## 1-5. 다이얼로그 — 시스템 알럿 대체 (v1.17, 2026-08-24 신설 · v1.22, 2026-08-24 개정 — 스크림 닫기 금지)
...
- mou `forms-list-client` 의 기존 기능 모달(div 오버레이·알럿 대체 아님) — `ModalShell`
  조합으로 후속 이행 후보.
- labs 수신자·mou 분원 담당자·popo 계정편집의 기능 모달 3곳 — 스크림 닫기는 v1.22 로
  제거했으나(§1-5 규칙 항) **자작 div 오버레이인 것은 그대로다.** forms-list 와 같은
  `ModalShell` 조합 이행 후보.
