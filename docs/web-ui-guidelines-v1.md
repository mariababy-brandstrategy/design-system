...
- **대가는 톤이 아니다 — `money`·`bulk` 는 승격 인자에서 뺀다**(v1.20). 그 자리를 무엇으로
  ...
  - **확인 게이트 의무 — 되돌릴 수 없는 일괄에만**(v1.21 에서 범위를 좁힘). `bulk` 이면서
    ...
    - ⚠ 전수조사 **F-6**(B4·B5 41곳 중 33곳 게이트 없음)·**F-7**(금전 15곳 원클릭)의 숫자는
      v1.20 기준의 계수다. v1.21 의 좁은 기준에서는 **그 숫자가 그대로 부채가 아니다** —
      위 두 조건으로 재판정한 것만 부채로 센다.
...
- **확인 게이트는 v1.21 에서 이 절의 의무가 됐다**(위 '확인 게이트 의무' 항) — 종전 문언은
  ...
  원칙은 그대로다.
  ⏳ **F-6(33곳)·F-7(15곳)의 계수는 v1.20 기준이라 현재 사실이 아니다** — v1.21 의 두 조건으로
  546행 전수를 다시 판정해야 실제 부채가 나온다. 재판정 전까지 그 숫자를 부채로 인용하지 않는다.
