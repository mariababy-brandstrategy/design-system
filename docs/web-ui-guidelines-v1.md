...
- **대가는 톤이 아니다 — `money`·`bulk` 는 승격 인자에서 뺀다**(v1.20). 그 자리를 무엇으로
  ...
  - **확인 게이트 의무 — 되돌릴 수 없는 일괄에만**(v1.21 에서 범위를 좁힘). `bulk` 이면서
    ...
    - ✅ **전수 재판정 완료(2026-08-24, S9)** — 546행을 위 두 조건으로 다시 셌다. **의무 4행 ·
      전부 해소**. 마지막 1행(sns 쓰다 만 내용 복구 = 전 칸 교체)은 게이트가 아니라 ↩ 복원으로
      ...
      **여러 입력 칸을 한 번에 채우거나 버리는 행동**이 빠져 있었다(위 조건 가의 예시로 든 sns
      "전부 새로 채우기"가 그중 하나였다). 옛 계수 F-6(41곳 중 33곳)·F-7(금전 15곳 원클릭)은
...
- **톤×강조 이행 완료(2026-08-24, S7 · v1.18~v1.20).** AGENTS.md "문서를 고칠 때" 절차 1~5 전부
  ...
  **claim 은 2026-08-22 앱 폐기로 제외**(`historical: true` — live 제외·static 유지).
  ...
  데이터를 지우지 않으므로 파괴 톤의 정의에 해당하지 않는다). 나머지 23행은 이미 원장에 있었다.
...
- **확인 게이트는 v1.21 에서 이 절의 의무가 됐다**(위 '확인 게이트 의무' 항) — 종전 문언은
  ...
  원칙은 그대로다.
  ✅ **재판정 완료(2026-08-24, S9)** — 546행 전수를 v1.21 두 조건으로 다시 셌다. 현재 부채는
  **0행**이다 — 마지막 1행(sns 쓰다 만 내용 복구 = 전 칸 교체)은 게이트가 아니라 ↩ 복원으로 닫았다.
...
- **v1.21 (2026-08-24)**: **v1.20 이 색에서 옮긴 부담의 착지 — 사용자 결정 3건.**
  ...
  기준 의무 4행·전부 해소(마지막 1행 = sns 쓰다 만 내용 복구, 게이트 대신 ↩ 복원). 그 과정에서 `bulk` 정의를 좁혀
