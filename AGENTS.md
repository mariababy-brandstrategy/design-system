...
- **소비자**(3): `docs/공간-브랜드-가이드-v1.0.{html,pdf}`(소스 `docs/space-brand-guide-src/`) · `docs/화면-브랜드-가이드-v1.0.{html,pdf}`(소스 `docs/screen-brand-guide-src/`, 2026-09-08 신설 — **외부 협력사·사내 개발용 화면 기준**) · `docs/v2.1-실무자용.{html,pdf}`
  (색상 가이드, 소스 `docs/color-guide-src/`, 2026-09-03 이전). base 를 고치면 소비자 전부를
  재조립해 렌더 diff 로 확인하고 이 목록을 갱신한다. 기계 검사는 두지 않는다(소비자 3건 — 정직 표기).
  ⚠ `styles/document-base.css` 머리 주석의 "현재 소비자" 줄은 2건에서 멈춰 있다(2026-09-08) — **소비자 목록의 정본은 이 절**이다.
  주석을 고치면 base 가 바뀌어 전 소비자 재조립이 걸리므로, 다음에 base 를 실제로 손볼 때 이 절을 가리키는 포인터로 정리한다.
- **폰트 서브셋에 글자를 늘릴 때**: `chars.txt` 에 넣는 것만으로는 부족하다 — **원본 Pretendard 에 그 글리프가 실제로 있어야** 한다
  ...
  `docs/screen-brand-guide-src/assemble.py` 의 커버리지 검사는 `chars.txt` 가 아니라 **woff2 의 실제 cmap** 을 본다 — 다른 조립기도 다음에 손볼 때 같은 방식으로 옮긴다.
