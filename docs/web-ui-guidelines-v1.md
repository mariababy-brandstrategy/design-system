...
## 1-4. 액션 색·위계 (v1.14, 2026-08-23 신설 · v1.18·v1.19, 2026-08-24 개정)
...
| **경고** | (기본 대상 없음 — 승격으로만 진입) | 확정 대상 중 **금전(money)·개인정보(pii)·제3자 업무 유발 알림(notify③)·일괄(bulk)·권한 경계 변경(authz)** 부수효과가 **하나라도** 있거나 **B4 또는 B5**(재입력 필요·복구 불가)인 것 |
...
- **플래그는 D-2 다섯 인자의 *입력*이지 여섯 번째 인자가 아니다.** 대응: `money`·`authz` → 영향 ·
  B4·B5 → 복구곤란도 · `notify③` → 외부파급 · `pii` → 민감정보 · `bulk` → 일괄범위.
...
- **권한 경계 변경(`authz`)은 승격 사유다** (Q4, 2026-08-24 사용자 결정). 역할·소속·접근 범위를
  ...
  인자로 사례마다 다투던 자리다.
  - **범위(좁은 정의)**: **실효 권한·역할·소속 범위를 부여·변경·회수**하는 행동에만 붙인다.
    권한 신청의 제출과, 실효 권한을 바꾸지 않는 **거부·반려에는 붙지 않는다** — 거부는 B4(재신청
    필요)로 이미 경고라 **톤 결론은 같고** 플래그 의미만 정확해진다(v1.18 판정, Codex 지적 반영).
...
- **"도구의 주행동"이라는 이유로 승격을 면제하지 않는다** (Q2, 2026-08-24 사용자 결정). 그 화면에서
  가장 자주 눌리는 대표 버튼이라도 `money`·B5 면 경고 톤이다 — 대표 첫 사례(popo-studio 생성 4버튼,
  유료 LLM 호출)를 예외로 만들면 승격 규칙이 문서로만 남는다. 반복 작업의 피로는 톤이 아니라
  **강조**로 조절한다(아래 강조 절). 색이 확인 게이트를 대체하지 않는다는 단서는 그대로다.
