...
| 우측 고정폭 요소 | 좁은 화면 분기 필수 | `hidden sm:flex` 등 (§6.1·§6.2) |

> **좁은 화면 규칙(2026-07-31)**: 우측 그룹은 `shrink-0` 이라 **거기 둔 고정폭은 화면이 좁아져도
> 그대로 자리를 먹는다.** 좌측 그룹은 `min-w-0` 이라 대신 줄어들다가 **폭 0 으로 붕괴**한다 —
> 워드마크·서비스명·탭이 통째로 사라지고 페이지가 가로로 넘친다(claim 320px 에서 172px 넘침,
> §6.1 실측). 우측에 고정폭(검색창·보조 링크 묶음)을 둘 땐 **좁은 화면 분기를 함께 둔다.**
> ⚠️ 이 규칙은 **정적 감사로 강제하지 않는다**(의도적 공백). 넘침 여부는 폰트·라벨 길이에 따라
> 달라져 렌더해야만 알 수 있고, 파일 이름만으로 "헤더 우측에 들어가는 컴포넌트"를 가려낼 수도
> 없다(hub `print-tools/tool-shell.tsx` 처럼 이름이 걸리는 무관 파일이 오검출된다).
> 대신 **헤드리스 실측 절차**를 쓴다 — maria-ui `docs/handoffs/2026-07-31-header-active-tab-scroll-and-aria-scope.md` §계측 방법.
...
`rightSlot`에 검색 인풋을 넣는다. 다크 배경 위 인풋 스타일 — **좁은 화면 분기가 규격의 일부다**:

```tsx
{/* 우측 그룹은 shrink-0 라 고정폭이 그대로 자리를 먹는다 — 좁은 화면에선 통째로 접는다. */}
<form className="hidden sm:flex items-center">
  <Input
    className="h-8 w-32 lg:w-[220px] border-maria-green-700 bg-maria-green-700 pl-7 text-xs text-text-on-dark placeholder:text-maria-green-300 focus-visible:border-text-on-dark focus-visible:ring-text-on-dark/30"
  />
</form>
```

> **220px 를 그냥 두면 안 되는 이유(2026-07-31 실측)**: 폭이 220px 로 고정된 채 `shrink-0` 우측
> 그룹 안에 있으면 320px 화면에서 우측만 451.9px 를 먹어(검색 220 + 보조 링크 151.8 + 아바타 32
> + 간격 48) **좌측 그룹이 폭 0 으로 붕괴**한다 — 워드마크·서비스명·탭이 통째로 사라지고 페이지가
> 가로로 172px 넘친다(375px 117 · 414px 78 · 768px 부터 정상). 검색을 아예 없애도 보조 링크만으로
> 219.8px 이라 부족하므로 **§6.2 와 함께 접어야** 한다.
> 접은 뒤: 320px 가로 넘침 0 · 탭 자리 73px · 활성 탭 100%. **1024px 이상은 변화 없음.**
> 검색 기능 자체는 `/lookup` 페이지에 그대로 있어 헤더에서만 접는 것이다.

### 6.2 관리자 전용 링크 (popo, claim 둘 다)

`secondaryLinks`에 조건부로 추가하거나, 상위에서 권한 검사 후 props로 전달.

**좁은 화면(태블릿 미만)에서는 숨긴다** — `hidden md:inline`. 링크 3개가 151.8px + 간격이라
검색을 접어도 이것만 남으면 320px 에서 좌측이 못 들어온다(§6.1 실측). 1차 탭과 브랜드가
헤더의 본질이고 보조 링크는 넓은 화면의 편의다.
⚠️ 접는 대가로 **좁은 화면에서는 그 링크가 유일 진입점이던 화면에 헤더로 못 간다.** claim 은
`사용 안내`·`휴지통` 이 여기 해당한다(사용자 승인 2026-07-31 — claim 은 입력 위주라 사실상 PC 앱).
새 앱에서 이 변형을 쓸 땐 그 화면에 다른 진입점이 있는지 먼저 볼 것.
...
- **v1.7 (2026-07-31)**: ① **활성 탭 자동 노출**(§2 규칙·§4 `useEffect`). 좁은 화면에서 메뉴가
  ...
  **헤더에서 색으로 현재 위치를 표시하는 모든 링크**. popo `관리자`, claim `사용 안내`·
  ...
  FAIL 로 잡는다.
- **v1.8 (2026-07-31)**: **좁은 화면 규칙**(§2 표·규칙 블록·§6.1·§6.2). 우측 그룹은 `shrink-0`
  이라 거기 둔 고정폭이 화면이 좁아져도 자리를 그대로 먹고, `min-w-0` 인 좌측이 대신 **폭 0 으로
  붕괴**한다. claim 실측: 320px 에서 우측만 451.9px(검색 220 + 보조 링크 151.8 + 아바타 32 +
  간격 48) → 워드마크·`Claims`·탭 5개가 통째로 사라지고 페이지가 가로로 **172px 넘침**
  (375px 117 · 414px 78 · 768px↑ 정상). 검색만 접어도 보조 링크 219.8px 이라 부족해 **둘 다**
  접는다 — 검색 `hidden sm:flex` + `w-32 lg:w-[220px]`, 보조 링크 `hidden md:inline`(사용자 승인).
